...
def Draw_connected_scene_graph(node_feature, relation, img_count, test_set, sg, idx,cnt_thres=2,view=True,save_path='./vis_result/'):
    ...
    if view and sg.format =='pdf':
        ...
        ratio=0.91
        if img.shape[1]<int(1920*ratio) and img.shape[0]<int(1080*ratio):
            pad = cv2.resize(img.copy(),(int(1920*ratio),int(1080*ratio)))
            pad.fill(255)
            pad[:img.shape[0], :img.shape[1], :] = img
            resized = pad
        elif img.shape[1]<int(1920*ratio):
            pad = cv2.resize(img.copy(), (int(1920 * ratio), int(1080 * ratio)))
            pad.fill(255)
            img = cv2.resize(img, (img.shape[1], int(1080 * ratio)))
            pad[:img.shape[0], :img.shape[1], :] = img
            resized = pad
        elif img.shape[0]<int(1080*ratio):
            pad = cv2.resize(img.copy(), (int(1920 * ratio), int(1080 * ratio)))
            pad.fill(255)
            img = cv2.resize(img, (int(1920 * ratio), img.shape[0]))
            pad[:img.shape[0], :img.shape[1], :] = img
            resized = pad
        else:
            resized = cv2.resize(img,(int(1920*ratio),int(1080*ratio)))
        cv2.namedWindow('3D Scene Graph',cv2.WINDOW_GUI_NORMAL)
        cv2.resizeWindow('3D Scene Graph', int(1920*ratio),int(1080*ratio))
